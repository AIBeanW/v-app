<template>
    <div class="v-upload" @click="changeInput">
        <input :multiple="multiple" class="v-upload_input" ref="file" @change="inputChange($event)" type="file">
        <slot></slot>
    </div>
</template>
<script>
        ...
        inputChange(event) {
            ...
            for (let i = 0; i < uploadFiles.length; i++) {
                ...
                if (
                    this.format &&
                    this.format.length &&
                    this.format.find(el => uploadFiles[i].type.indexOf(el) !== -1) === undefined
                ) {
                    this.error(
                        {
                            msg: "该类型不支持上传~"
                ...
                this.beforeUpload(uploadFiles[i]) === false ? "" : this.upload(uploadFiles[i]);
            }
        }
    }
};
